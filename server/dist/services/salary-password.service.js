...
export async function verifySalaryPassword(userId, password, app, ip, userAgent) {
    ...
    if (record.lockedUntil && record.lockedUntil > new Date()) {
        throw new SalaryPasswordError(`密码错误次数过多，请 ${LOCK_DURATION_MINUTES} 分钟后重试`, 21002, 423);
    }
    const valid = await verifyPassword(password, record.passwordHash);
    if (!valid) {
        const newFailCount = record.failCount + 1;
        const lockedUntil = newFailCount >= MAX_FAILS
        ...
        if (lockedUntil) {
            throw new SalaryPasswordError(`密码错误次数过多，已锁定 ${LOCK_DURATION_MINUTES} 分钟`, 21002, 423);
        }
    ...
    const token = await app.jwt.sign({ userId, type: 'salary' }, { expiresIn: TOKEN_TTL_SECONDS + 's' });
    await prisma.salaryAuditLog.create({
        data: {
            userId,
...
export async function setSalaryPassword(userId, oldPassword, newPassword) {
...
//# sourceMappingURL=salary-password.service.js.map