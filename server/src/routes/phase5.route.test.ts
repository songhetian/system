...
describe('Phase 5 Routes - Expense', () => {
  ...
  afterEach(async () => {
    for (const id of createdClaimIds) {
      try { await prisma.expenseClaim.delete({ where: { id } }); } catch {}
    }
...
describe('Phase 5 Routes - Training', () => {
  ...
  afterEach(async () => {
    for (const id of createdEnrollmentIds) {
      try { await prisma.trainingEnrollment.delete({ where: { id } }); } catch {}
    }
    createdEnrollmentIds.length = 0;
    for (const id of createdCourseIds) {
      try { await prisma.trainingCourse.delete({ where: { id } }); } catch {}
    }
