...
describe('Employee Routes', () => {
    ...
    const createEmployeePayload = (positionId, suffix) => ({
    ...
    it('GET /api/employees - 分页查询员工列表', async () => {
        ...
        const item = body.data.list.find((e) => e.id === e1.id);
    ...
    it('GET /api/employees - 按关键词搜索', async () => {
        ...
        const body = JSON.parse(response.body);
        expect(body.code).toBe(0);
        expect(body.data.list.some((e) => e.name === uniqueName)).toBe(true);
    });
    it('GET /api/employees - 按状态过滤', async () => {
        ...
        const body = JSON.parse(response.body);
        expect(body.code).toBe(0);
        expect(body.data.list.every((e) => e.status === 'ACTIVE')).toBe(true);
    });
    // ─── 获取员工详情 ──────────────────────────────────────────
...
//# sourceMappingURL=employee.route.test.js.map