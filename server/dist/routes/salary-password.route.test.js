...
describe('Salary Password Routes - 二级密码', () => {
    ...
    const generateAccessToken = async (app, userId, username) => {
        return app.jwt.sign({ id: userId, username, employeeId: testEmployeeId }, { expiresIn: '15m' });
    };
    afterEach(async () => {
        if (testUserId) {
            try {
                await prisma.salaryPassword.deleteMany({ where: { userId: testUserId } });
            }
            catch { }
            try {
                await prisma.salaryAuditLog.deleteMany({ where: { userId: testUserId } });
            }
            catch { }
            try {
                await prisma.userRoles.deleteMany({ where: { userId: testUserId } });
            }
            catch { }
            try {
...
//# sourceMappingURL=salary-password.route.test.js.map