...
export async function createRotationRule(data: RotationRuleCreate) {
  if (data.pattern.length !== data.cycleDays) {
    throw new RotationRuleError(
      `pattern 长度 (${data.pattern.length}) 必须等于 cycleDays (${data.cycleDays})`,
      30101,
      400,
    );
  }
...
export async function findRotationRuleById(id: number) {
  return await prisma.rotationRule.findUnique({ where: { id } });
}

export async function updateRotationRule(id: number, data: Partial<RotationRuleCreate>) {
...
export async function deleteRotationRule(id: number) {
  try {
