...
import { createSchedule, findSchedulesByDateRange, deleteSchedule, checkConflicts, generateSchedule, } from './schedule.service.js';
describe('Schedule Service', () => {
    ...
    describe('generateSchedule - 排班生成', () => {
        ...
        afterEach(async () => {
            try {
                await prisma.schedule.deleteMany({ where: { employeeId: { in: [employeeId, emp2Id] } } });
            }
            catch { }
            try {
                await prisma.employeePosition.deleteMany({ where: { employeeId: { in: [employeeId, emp2Id] } } });
            }
            catch { }
            try {
                await prisma.employee.delete({ where: { id: emp2Id } });
            }
            catch { }
            try {
                await prisma.rotationRule.delete({ where: { id: rotationRuleId } });
            }
            catch { }
            try {
            ...
            try {
                await prisma.rank.deleteMany({ where: { level: { gte: 999 } } });
            }
            catch { }
        });
