...
describe('Row-Level Permission - 行级数据权限', () => {
  ...
  let userA2Id: number;
  let userB1Id: number;
  let leaveRoleId: number;

  const buildAppWithScope = async (userId: number, employeeId: number, scope: string) => {
    // 清缓存，确保每次读取最新数据权限
  ...
  beforeAll(async () => {
    ...
    if (!perm) perm = await prisma.permission.create({ data: { name: '请假查看', code: 'leave:read', group: 'leave' } });
    let role = await prisma.role.findUnique({ where: { code: 'leave_read' } });
    if (!role) role = await prisma.role.create({ data: { name: '请假查看', code: 'leave_read' } });
    leaveRoleId = role.id;
  ...
  it('DEPARTMENT 范围：部门主管能看到本部门所有请假', async () => {
    ...
    const body = JSON.parse(response.body);
    // 部门A有2个员工，应该返回2条
    const empIds = body.data.list.map((r: any) => r.employeeId);
