...
export async function createDataPermission(data: DataPermissionCreate) {
...
export async function listDataPermissions(query: DataPermissionQuery) {
...
export async function getDataPermission(id: number) {
  ...
  if (!perm) throw Errors.notFound('数据权限配置不存在');
  return perm;
}

export async function updateDataPermission(id: number, data: DataPermissionUpdate) {
...
export async function getUserDataScope(userId: number, resourceType: string): Promise<string> {
  const userPerm = await prisma.dataPermission.findFirst({
    where: { userId, resourceType },
  });

  if (userPerm) return userPerm.scope;
  ...
  if (roleIds.length === 0) return 'OWN';
...
export async function getUserDepartmentIds(userId: number, scope: string): Promise<number[]> {
  if (scope === 'ALL') return [];
  if (scope === 'OWN') return [];
