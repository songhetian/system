...
export async function checkScheduleConstraints(employeeId, shiftTemplateId, date, config = {}) {
    const mergedConfig = {
    ...
    const sameDaySchedule = existingSchedules.find((s) => s.date.getTime() === dayStart.getTime() && s.shiftTemplateId !== shiftTemplateId);
    if (sameDaySchedule) {
        violations.push({
            type: 'HARD',
    ...
    const weekSchedules = existingSchedules.filter((s) => s.date >= weekStart && s.date <= weekEnd);
    ...
    const prevSchedule = existingSchedules.find((s) => s.date.getTime() === prevDay.getTime());
    ...
    const targetDayIdx = sortedSchedules.findIndex((s) => s.date.getTime() === dayStart.getTime());
...
export async function checkBatchScheduleConstraints(schedules, config) {
    ...
    for (const sched of schedules) {
        ...
        const violations = checkScheduleConstraintsWithData(sched.employeeId, templateMap.get(sched.shiftTemplateId) || null, schedulesByEmployee.get(sched.employeeId) || [], sched.date, config);
        result.set(key, violations);
    }
    return result;
}
function checkScheduleConstraintsWithData(employeeId, shiftTemplate, existingSchedules, date, config = {}) {
    if (!shiftTemplate)
        return [];
    const mergedConfig = {
        hardConstraints: { ...DEFAULT_CONFIG.hardConstraints, ...config.hardConstraints },
    ...
    const dayStart = new Date(date);
    dayStart.setHours(0, 0, 0, 0);
    const sameDaySchedule = existingSchedules.find((s) => new Date(s.date).getTime() === dayStart.getTime() && s.shiftTemplateId !== shiftTemplate.id);
    if (sameDaySchedule) {
        violations.push({
    ...
    const weekStart = getStartOfWeek(date);
    const weekEnd = getEndOfWeek(weekStart);
    const weekSchedules = existingSchedules.filter((s) => new Date(s.date) >= weekStart && new Date(s.date) <= weekEnd);
    const weekHours = weekSchedules.reduce((sum, s) => {
        if (s.shiftTemplate)
            return sum + calculateShiftHours(s.shiftTemplate.startTime, s.shiftTemplate.endTime);
        return sum;
...
//# sourceMappingURL=schedule-constraints.service.js.map