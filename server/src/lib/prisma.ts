...
export function setAuditContext(userId: number | null, username: string | null, ip?: string | null) {
...
prismaClient.$use(async (params, next) => {
  ...
  if (
    ...
    const actionMap = { create: 'CREATE', update: 'UPDATE', delete: 'DELETE' } as const;
    ...
    try {
      await prismaClient.operationLog.create({
        data: {
          ...
          action: actionMap[params.action] as any,
          entityType: params.model,
          entityId: (result as any)?.id || params.args?.where?.id || 0,
