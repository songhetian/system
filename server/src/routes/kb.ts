...
import {
  ...
  fileSecurityLevels,
} from '../services/file-security.service.js';
...
function toDocumentView(doc: any) {
  return {
    ...doc,
...
export const kbRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post(
    ...
    async (request, reply) => {
      try {
        ...
        if (!data) throw new Error('未上传文件');

        // 解析额外表单字段
        for (const field in data.fields) {
          if (field === 'title') title = (data.fields[field] as any)?.value || '';
          if (field === 'category') category = (data.fields[field] as any)?.value || 'OTHER';
        }
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        const doc = await updateDocumentSecurity(
          Number(id),
          body.securityLevel,
          body.allowedUserIds,
          body.allowedRoleIds,
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      const secondPasswordVerifiedAt = Number((request.headers as any)['x-second-password-verified-at']) || undefined;
