...
describe('Performance Baseline Tests', () => {
    ...
    test('并发 50 个员工查询请求 ≤5s 总耗时', async () => {
        const start = Date.now();
        const requests = Array(50).fill(null).map(() => app.inject({ method: 'GET', url: '/api/employees?page=1&pageSize=10' }));
    ...
    test('并发 20 个部门查询请求 ≤2s 总耗时', async () => {
        const start = Date.now();
        const requests = Array(20).fill(null).map(() => app.inject({ method: 'GET', url: '/api/org/departments?page=1&pageSize=10' }));
        const responses = await Promise.all(requests);
        const duration = Date.now() - start;
        responses.forEach(r => expect(r.statusCode).toBe(200));
