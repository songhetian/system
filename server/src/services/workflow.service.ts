...
import type { WorkflowTemplateCreate, WorkflowTemplateUpdate, TemplateNode, Edge, WorkflowInstanceCreate } from '@shop/shared';

// 审批流回调 — 业务模块注册自己的审批通过/驳回处理逻辑
interface WorkflowCallbacks {
  onApproved?: (sourceId: number, tx: any) => Promise<void>;
  onRejected?: (sourceId: number, tx: any, reason?: string) => Promise<void>;
}

const callbacks = new Map<string, WorkflowCallbacks>();

export function registerWorkflowCallback(sourceType: string, cb: WorkflowCallbacks) {
  callbacks.set(sourceType, cb);
}

export async function getWorkflowTemplateBySourceType(sourceType: string) {
...
export async function createWorkflowTemplate(data: WorkflowTemplateCreate) {
  return prisma.workflowTemplate.create({
    data: {
      name: data.name,
      description: data.description,
      nodes: data.nodes as TemplateNode[],
...
export async function listWorkflowTemplates(page: number, pageSize: number) {
...
export async function updateWorkflowTemplate(id: number, data: Partial<WorkflowTemplateUpdate>) {
  return prisma.workflowTemplate.update({
    where: { id },
    data: {
      ...(data.name !== undefined && { name: data.name }),
      ...(data.description !== undefined && { description: data.description }),
      ...(data.nodes !== undefined && { nodes: data.nodes as TemplateNode[] }),
      ...(data.edges !== undefined && { edges: data.edges as Edge[] }),
    },
  });
}
...
async function resolveAssignee(
  ...
  node: TemplateNode,
  applicantId: number,
): Promise<{ assigneeId: number | null; assigneeName: string | null }> {
...
export async function createWorkflowInstance(data: WorkflowInstanceCreate & { applicantId?: number }) {
  const template = await prisma.workflowTemplate.findUnique({
    where: { id: data.templateId, status: 'PUBLISHED' },
  });
  if (!template) throw Errors.notFound('模板不存在或未发布');
...
export async function listWorkflowInstances(page: number, pageSize: number) {
  const [list, total] = await Promise.all([
...
export async function approveWorkflowInstance(id: number, comment: string) {
  return prisma.$transaction(async (tx: any) => {
    // ponytail: Prisma 5.22 不支持 findUnique.lock，用原生 FOR UPDATE 手动加行锁防止并发审批
    const [locked] = await tx.$queryRawUnsafe(
      `SELECT id FROM WorkflowInstance WHERE id = ? FOR UPDATE`,
      id,
    );
    if (!locked) throw Errors.notFound('实例不存在');

    const instance = await tx.workflowInstance.findUnique({
      where: { id },
      include: { nodes: { orderBy: { nodeIndex: 'asc' } } },
    });
    if (!instance) throw Errors.notFound('实例不存在');
    if (instance.status !== 'IN_PROGRESS') {
      throw Errors.business('审批已结束，无法重复操作');
    }

    const pendingNode = instance.nodes.find(
      (n) => n.type === 'APPROVAL' && n.status === 'PENDING',
    );
    if (!pendingNode) throw Errors.business('没有待审批节点');
    ...
    if (newStatus === 'APPROVED') {
      ...
      const nextPending = instance.nodes.find(
        (n, i) => i > pendingNode.nodeIndex && n.type === 'APPROVAL' && n.status === 'PENDING',
      );
...
export async function rejectWorkflowInstance(id: number, reason: string) {
  return prisma.$transaction(async (tx: any) => {
...
export async function returnWorkflowInstance(
  ...
  return prisma.$transaction(async (tx: any) => {
    ...
    const currentPendingIndex = instance.nodes.findIndex(
      (n) => n.type === 'APPROVAL' && n.status === 'PENDING',
    );
    if (currentPendingIndex === -1) throw Errors.business('没有待审批节点');
    ...
    if (targetType === 'PREVIOUS') {
      ...
      if (targetNodeIndex === undefined) throw Errors.validation('必须指定退回节点');
