...
export async function createEmployee(data: EmployeeCreate) {
  ...
  if (!dept) throw Errors.notFound('部门不存在');
  ...
  if (wrongDept) throw Errors.business('岗位不属于所选部门');
...
export async function listEmployees(params: {
  ...
  if (positionId || departmentId) {
    const posFilter: any = {};
    if (positionId) posFilter.positionId = positionId;
    if (departmentId) posFilter.position = { departmentId };
...
export async function regularizeEmployee(id: number, regularizeDate: string) {
...
export async function resignEmployee(id: number, resignDate: string, reason: string) {
  const emp = await prisma.employee.findUnique({ where: { id, deletedAt: null } });
  if (!emp) throw Errors.notFound('员工不存在');
