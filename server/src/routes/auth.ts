...
export const authRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        const decoded = await app.jwt.verify<{ id: number; type: string }>(refreshToken);
        ...
        const accessToken = await app.jwt.sign(
          { id: user.id, username: user.username, employeeId: user.employeeId },
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!user) return reply.status(404).send({ code: 10000, data: null, message: '用户不存在' });

      const valid = await bcrypt.compare(oldPassword, user.password);
      if (!valid) return reply.status(400).send({ code: 10000, data: null, message: '旧密码错误' });
