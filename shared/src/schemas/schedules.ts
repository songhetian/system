...
export const zRotationRuleCreateBase = z.object({
  name: z.string().min(1).max(50),
  pattern: z.array(zRotationPattern).min(1),
  cycleDays: z.number().int().min(1),
});

export const zRotationRuleCreate = zRotationRuleCreateBase.superRefine((data, ctx) => {
...
export const zRotationRule = zRotationRuleCreateBase
  .extend({ id: z.number().int().positive() })
  .merge(auditMixin);
