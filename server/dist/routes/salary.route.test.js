...
describe('Salary Routes - 薪资结构', () => {
    ...
    afterEach(async () => {
        ...
        for (const id of createdAssignmentIds) {
            try {
                await prisma.salaryStructureAssignment.delete({ where: { id } });
            }
            catch { }
        }
        ...
        for (const id of createdEmployeeIds) {
            try {
                await prisma.employeePosition.deleteMany({ where: { employeeId: id } });
            }
            catch { }
            try {
...
describe('Salary Routes - 工资条', () => {
    ...
    it('GET /api/salary/payslips - 工资条列表（金额脱敏）', async () => {
        ...
        expect(body.code).toBe(0);
        expect(Array.isArray(body.data.list)).toBe(true);
        const item = body.data.list.find((p) => p.id === payslip.id);
...
//# sourceMappingURL=salary.route.test.js.map