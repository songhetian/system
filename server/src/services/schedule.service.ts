...
export async function createSchedule(data: ScheduleCreate) {
  try {
...
export async function updateSchedule(id: number, data: ScheduleUpdate) {
  try {
...
export async function generateSchedule(data: ScheduleGenerateInput): Promise<ScheduleGenerateResult> {
  ...
  const pattern = rotationRule.pattern as Array<{ dayOffset: number; shiftTemplateId: number }>;
  ...
  const leaveMap = new Map<number, Array<{ start: Date; end: Date }>>();
  ...
  function hasLeave(empId: number, date: Date): boolean {
    const leaves = leaveMap.get(empId);
    if (!leaves) return false;
    return leaves.some((l) => l.start <= date && l.end >= date);
  }

  const toCreate: Array<{ employeeId: number; shiftTemplateId: number; date: Date }> = [];
  let totalAssigned = 0;
  let conflicting = 0;
  let skippedLeave = 0;
...
export async function clearScheduleRange(data: ScheduleClearRange) {
  const where: any = {
    date: {
...
export async function checkConflicts(params: {
  ...
  const scheduleMap = new Map<string, Array<any>>();
  ...
  for (const [key, items] of scheduleMap) {
    if (items.length > 1) {
      ...
      conflicts.push({
        ...
        date: dateStr,
        conflicts: items.map((s: any) => s.shiftTemplate.name),
      });
    }
  }
