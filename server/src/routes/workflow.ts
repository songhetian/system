...
function toTemplateView(template: any) {
  return {
    ...template,
...
export const workflowRoutes: FastifyPluginAsync = async (app) => {
  ...
  app.put(
    ...
    async (request, reply) => {
      const { id } = request.params as any;
      try {
        const template = await updateWorkflowTemplate(Number(id), request.body as any);
        return { code: 0, data: toTemplateView(template), message: 'success' };
      } catch (err: any) {
        reply.status(400);
        return { code: 10000, data: null, message: err.message };
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        const instance = await returnWorkflowInstance(
          Number(id),
          body.targetType,
          body.reason || '',
