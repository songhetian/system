...
export async function createOvertimeRequest(data: OvertimeRequestCreate) {
...
export async function listOvertimeRequests(query: OvertimeRequestQuery) {
...
export async function getOvertimeRequest(id: number) {
  const request = await prisma.overtimeRequest.findUnique({ where: { id } });
  if (!request) throw Errors.notFound('加班申请不存在');
  return request;
}

export async function updateOvertimeRequest(id: number, data: OvertimeRequestUpdate) {
...
export async function approveOvertimeRequest(id: number, approvedHours?: number) {
  const request = await getOvertimeRequest(id);
  if (request.status !== 'PENDING') {
...
export async function rejectOvertimeRequest(id: number, reason?: string) {
  const request = await getOvertimeRequest(id);
  if (request.status !== 'PENDING') {
...
export async function createOvertimeRecord(data: OvertimeRecordCreate) {
...
export async function listOvertimeRecords(employeeId?: number, requestId?: number) {
  const where: any = {};
  if (employeeId) where.employeeId = employeeId;
