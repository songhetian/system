...
export async function generate(data: GenerateInput) {
  ...
  const recordMap = new Map<number, Array<{ date: number; type: string; lateMinutes: number; earlyMinutes: number }>>();
  ...
  for (const employee of employees) {
    ...
    for (const record of empRecords) {
      ...
      if (record.type === 'IN' && record.lateMinutes > 0) lateCount++;
      if (record.type === 'OUT' && record.earlyMinutes > 0) earlyCount++;
    }
...
export async function lock(id: number) {
