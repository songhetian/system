...
describe('Role Routes - 角色', () => {
  ...
  const uniqueCode = (prefix = 'r') =>
    `${prefix}_${Array.from({ length: 8 }, () =>
  ...
  afterEach(async () => {
    ...
    for (const id of createdRoleIds) {
      try { await prisma.rolePermission.deleteMany({ where: { roleId: id } }); } catch {}
      try { await prisma.role.update({ where: { id }, data: { deletedAt: new Date() } }); } catch {}
      try { await prisma.role.delete({ where: { id } }); } catch {}
    }
    createdRoleIds.length = 0;

    for (const id of createdPermissionIds) {
      try { await prisma.permission.delete({ where: { id } }); } catch {}
    }
  ...
  it('GET /api/roles - 按关键词搜索', async () => {
    ...
    const body = JSON.parse(response.body);
    expect(body.code).toBe(0);
    expect(body.data.list.some((item: any) => item.name === keyword)).toBe(true);
  });
...
describe('Permission Routes - 权限', () => {
  ...
  it('GET /api/permissions - 查询所有权限', async () => {
    ...
    expect(body.data.some((item: any) => item.id === p.id)).toBe(true);
  });
  ...
  it('GET /api/permissions?group=xxx - 按 group 过滤', async () => {
    ...
    const body = JSON.parse(response.body);
    expect(body.code).toBe(0);
    expect(body.data.every((item: any) => item.group === '过滤组')).toBe(true);
  });
});
...
describe('User Routes - 用户角色', () => {
  ...
  const uniqueCode = () =>
    `r_${Array.from({ length: 8 }, () => String.fromCharCode(97 + Math.floor(Math.random() * 26))).join('')}`;
