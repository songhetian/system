...
export const dashboardStatsRoutes: FastifyPluginAsync = async (app) => {
  app.get('/', async () => {
    ...
    const deptChart = departments.map((d) => ({
      name: d.name,
      count: d.positions.reduce(
        (sum, p) =>
          sum + p.employeePositions.filter((ep) => ep.employee.status === 'ACTIVE').length,
        0,
      ),
    }));
