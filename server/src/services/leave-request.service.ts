...
function calculateLeaveDays(
  ...
  let adjust = 0;
  if (startTime === 'PM') adjust += 0.5;
  if (endTime === 'AM') adjust += 0.5;
...
export async function findLeaveRequestByFilter(params: LeaveRequestQuery, extraWhere?: Record<string, any>) {
...
export async function findLeaveRequestById(id: number) {
  ...
  return {
    ...request,
    approvalChain: approvalChain ? JSON.parse(approvalChain.nodes as string) : null,
  };
}

export async function updateLeaveRequest(id: number, data: LeaveRequestUpdate) {
  const request = await prisma.leaveRequest.findUnique({
    where: { id },
  });
...
export async function approveLeaveRequest(id: number, data: LeaveApproval) {
  return prisma.$transaction(async (tx: any) => {
    // ponytail: 先锁 leaveRequest 行，防止并发重复审批
    const [locked] = await tx.$queryRawUnsafe(
      `SELECT id, status FROM LeaveRequest WHERE id = ? FOR UPDATE`,
      id,
    );
    ...
    if (typeKey) {
      ...
      const [quota] = await tx.$queryRawUnsafe(
        `SELECT id FROM LeaveQuota WHERE employeeId = ? AND year = ? FOR UPDATE`,
        request.employeeId,
        year,
      );
      if (!quota) {
...
export async function rejectLeaveRequest(id: number, data: LeaveRejection) {
  const request = await prisma.leaveRequest.findUnique({
    where: { id },
  });
