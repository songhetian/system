import { z } from 'zod';
import { zEmployeeCreate, zEmployeeUpdate, zEmployeeListItem, zEmployeeFullDetail, zEmployeeQuery, zRegularizeInput, zResignInput, } from '@shop/shared';
import { createResponseSchema, createPaginatedResponseSchema, zodToSchema } from '../utils/zod-to-schema.js';
import { createEmployee, getEmployeeById, listEmployees, updateEmployee, deleteEmployee, regularizeEmployee, resignEmployee, } from '../services/employee.service.js';
...
function toListItem(emp) {
    return {
        ...
        idCard: '****',
        phone: '****',
        positionIds: emp.employeePositions?.map((ep) => ep.positionId) ?? [],
        departmentId: emp.employeePositions?.[0]?.position?.departmentId ?? 0,
    };
}
function toFullDetail(emp) {
    return {
        ...emp,
        hireDate: emp.hireDate?.toISOString().split('T')[0] ?? null,
        regularizeDate: emp.regularizeDate?.toISOString().split('T')[0] ?? null,
...
export const employeeRoutes = async (app) => {
