...
describe('Workflow Routes - 流程模板', () => {
    ...
    afterEach(async () => {
        for (const id of createdInstanceIds) {
            try {
                await prisma.workflowInstanceNode.deleteMany({ where: { instanceId: id } });
            }
            catch { }
            try {
        ...
        for (const id of createdTemplateIds) {
            try {
                await prisma.workflowTemplate.update({ where: { id }, data: { deletedAt: new Date() } });
            }
            catch { }
            try {
...
describe('Workflow Routes - 审批实例', () => {
    ...
    it('POST /api/workflow/instances/:id/approve - 并发审批同一节点只生效一次', async () => {
        ...
        const failed = results.filter((r) => r.status === 'fulfilled' && r.value.statusCode !== 200);
...
//# sourceMappingURL=workflow.route.test.js.map